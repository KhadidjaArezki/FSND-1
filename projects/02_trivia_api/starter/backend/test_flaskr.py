...
class TestTrivia(unittest.TestCase):
    ...
    def test_create_question_422(self):
        ...
        response = self.client().post('/questions', json={'question': 'Is this good', 'answer':'NO'})
        response_data = json.loads(response.data)
    ...
    #     self.assertEqual(response_data['deleted'], 24)
    #     self.assertEqual(response_data['success'], True)
    ...
    def test_get_category_questions_200(self):
        """ Response must return 200 - a set of questions of the specified category"""
    ...
    def test_save_game_422(self):
        """ Response must return an  Unprocessible error"""
        response = self.client().post('/games', json={'score': 10, 'name':'best_player'})
        response_data = json.loads(response.data)
