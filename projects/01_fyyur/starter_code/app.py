    ...
    try:
        ...
        for city in cities:
            ...
            for venue in venues:
                ...
                next_shows_count = Show.query.filter_by(venue_id=venue.id).filter(
                    db.text('start_time > now()')).count()
...
def search_venues():
    ...
    try:
        ...
        for venue in venues:
            ...
            next_shows_count = Show.query.filter_by(artist_id=venue.id).filter(
                db.text('start_time > now()')).count()
...
def search_artists():
    ...
    try:
        ...
        for artist in artists:
            ...
            artist_obj['id'] = artist.id
            artist_obj['name'] = artist.name
            next_shows_count = Show.query.filter_by(artist_id=artist.id).filter(
                db.text('start_time > now()')).count()
...
def show_artist(artist_id):
    ...
    try:
        ...
        past_shows = []
        old_shows = Show.query.filter_by(artist_id=artist.id).filter(
            db.text('start_time < now()')).all()
        data['past_shows_count'] = len(old_shows)
